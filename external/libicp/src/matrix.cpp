...
Matrix Matrix::rotFromAngle(const FLOAT &angle) {
    FLOAT s = sin(angle);
    FLOAT c = cos(angle);
    Matrix R(3,3);
    ...
    R.val[0][0] = (1-c)*e1*e1 + c;      R.val[0][1] = (1-c)*e1*e2 - e3 * s; R.val[0][2] = (1-c)*e1*e3 + e2 * s;
    R.val[1][0] = (1-c)*e2*e1 + e3 * s; R.val[1][1] = (1-c)*e2*e2 + c;      R.val[1][2] = (1-c)*e2*e3 - e1 * s;
    R.val[2][0] = (1-c)*e3*e1 - e2 * s; R.val[2][1] = (1-c)*e3*e2 + e1 * s; R.val[2][2] = (1-c)*e3*e3 + c;

...
double IcpPointToPlane::getInitialGuess(double *T, const int32_t T_num, const std::vector<std::pair<int, int> > &correspondences, Matrix &R, Matrix &t)
{
	int i;
	int nact = (int) correspondences.size();
	...
	double mut0 = 0.0, mut1 = 0.0, mut2 = 0.0;
	  ...
	  // establish correspondences
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,A,b) // schedule (dynamic,2)
	for (i = 0; i < nact; i++) {
		  // set model point
		p_m.val[i][0] = M_tree->the_data[correspondences[i].first][0]; mum0 += p_m.val[i][0];
		p_m.val[i][1] = M_tree->the_data[correspondences[i].first][1]; mum1 += p_m.val[i][1];
		p_m.val[i][2] = M_tree->the_data[correspondences[i].first][2]; mum2 += p_m.val[i][2];

		  // set template point
		p_t.val[i][0] = T[3 * correspondences[i].second + 0]; mut0 += p_t.val[i][0];
		p_t.val[i][1] = T[3 * correspondences[i].second + 1]; mut1 += p_t.val[i][1];
		p_t.val[i][2] = T[3 * correspondences[i].second + 2]; mut2 += p_t.val[i][2];
	}
	...
	mu_t.val[0][2] = mut2;
	  ...
	  // subtract mean
	mu_m = mu_m / (double)nact;
	mu_t = mu_t / (double)nact;
	Matrix q_m = p_m - Matrix::ones(nact, 1) * mu_m;
	Matrix q_t = p_t - Matrix::ones(nact, 1) * mu_t;

...
public:
	...
	IcpPointToPlane (double *M, const int32_t M_num, const int32_t dim, const std::string save_dir = "", const int32_t num_neighbors = 10, const double flatness = 5.0) : Icp(M, M_num, dim, save_dir)
	{
	...
	double getInitialGuess(double *T, const int32_t T_num, const std::vector<std::pair<int, int> > &correspondences, Matrix &R, Matrix &t);

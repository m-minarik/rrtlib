# -*- coding: utf-8 -*-
"""
Visualizes all object trajectories by placing cylinders along the paths.
...
def visualize_trajectory(run_dir: str):
    ...
    # cam_ob.rotation_euler = (1.012291, 0, 2.042035)  # 58, 0, 117
    # Two walls
    # cam_ob.location = (10, 23, 17)
    # cam_ob.rotation_euler = (1.012291, 0, 2.722714)  # 58, 0, 156
    # Overhead view
    cam_ob.location = (0, 0, 25)
    cam_ob.rotation_euler = (0, 0, 3.141593)  # 58, 0, 156
    ...
    start = import_off(info["params"]["object"], color=(0, 0.01, 0.8, 1))
    goal = import_off(info["params"]["object"], color=(0, 0.01, 0.8, 1))

    # Convert string to list of floats
    start_conf = list(map(float, info["params"]["start"].split()))
    start.location = start_conf[0:3]
    start.rotation_euler = start_conf[3:6]

    goal_conf = list(map(float, info["params"]["goal"].split()))
    goal.location = goal_conf[0:3]
    goal.rotation_euler = goal_conf[3:6]

    # Create visualization
    p1 = None
    p2 = None
    s = 0
    for path_id in info["paths"].keys():
        print(f"Loading {path_id}")
        with open(info["paths"][f"{path_id}"]["path_filename"], mode="r") as f:
            # Number of steps
            total_steps = int(f.readline().strip())
            p2 = list(map(float, f.readline().strip().split()))[0:3]

            for line in f:
                # Connects each 5th point by a cylinder
                if s < 5:
                    s += 1
                    continue
                s = 0
